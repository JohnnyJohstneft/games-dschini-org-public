...
class ItemProxy
{
	...
	public static function getAllOfAuthor( $facebook_id, $game_id=9 ){
                $sql = sprintf("SELECT i.*,ia.data FROM items i, items_data ia WHERE ia.item_id=i.id AND i.author='%s' AND i.game_id=%d",$facebook_id,$game_id);
	...
	public static function getAllOfUser( $facebook_id, $game_id=9 ){
		$sql = sprintf("SELECT i.*,fi.facebook_id,fi.created,fi.amount FROM fbaccount_items fi, items i WHERE fi.item_id=i.id AND fi.facebook_id='%s' AND i.game_id=%d",$facebook_id,$game_id);
